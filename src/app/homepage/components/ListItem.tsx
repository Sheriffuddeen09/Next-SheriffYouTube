...
type Param = {
    postPost: Promise<Posts[]>
}
export default async function ListItem({postPost}:Param) {
    ...
    const posts = await postPost

    const content = (
        <main className="p-3 grid gap-2 grid-cols-1 mt-28 lg:grid-cols-3 md:grid-cols-1 sm:ml-10">
          {
            posts.map((post) =>(
              <div>
