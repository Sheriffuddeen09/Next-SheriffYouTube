...
export default function ShortItem(short:Shorts){
...
Send feedback</p>
    ...
    return(
      ...
      <div className='mx-auto flex flex-col items-center justify-center'>
        ...
        <Video src={short.video} width={290} height={450} 
        className='rounded-xl block sm:hidden -translate-y-5'/>
        <div className='relative -top-24'>
            <div className='inline-flex items-center gap-2 ml-5'>
            <Image src={`/${short.user}`} alt='imagepic' className='rounded-full' width={40} height={40} />
            <p className='font-bold text-sm'>{short.name}</p>
            <button onClick={handleSubscribe} className="inline-flex ">
    <p className={`bg-white text-black py-1 px-3 rounded-3xl ${subscribe ? "hidden" : "block"}`} style={{fontSize:"11px"}}>{short.subscribe}</p>
...
{short.subscribed} <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="size-4">
    <path stroke-linecap="round" stroke-linejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" />
            ...
            <div className='bg-black  pt-6 pb-16 rounded-b-xl mt-1'>
            <p className='font-bold text-sm  ml-3'>.{short.title}</p>
            <p className='w-72 mt-5 text-sm  ml-3 font-bold'>{short.body}</p>
        </div>
        </div>
        <div>
        </div>
    </div>
    <div className='relative right-24 sm:right-0'>
        {sidecontent}
    </div>
    </div>
    )
