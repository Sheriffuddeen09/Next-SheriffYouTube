...
export default function SubscribeLists(post:Posts){
      ...
      <div>
        ...
        <Link href={`/subscribeid/${post.id}`}>
         ...
         <span className='w-64 block sm:hidden'>
             <Video src={post.video} width={240} className='bg-gray-400 mb-2 w-40 md:w-32 h-60 rounded-xl p-1'
             />
               
         </span>
        </Link>
         <div className=''>
        <div className='inline-flex sm:gap-20 gap-3 '>
        ...
        <p className='text-sm block sm:hidden'>
            { 
                (post.title).length <= 10 ?
                post.title : `${(post.title).slice(0,18)}...`
