...
export default function SubscribeList(post:Posts){
    ...
    return(
      <div>
        ...
        <div className='flex flex-col sm:flex-row gap-4 my-1.5 text-sm sm:translate-x-0 -translate-x-2'>
        <Link href={`/edit/${post.id}`}>
         ...
         <span className='w-72 block sm:hidden'>
             <Video src={post.video} width={285} className='bg-gray-400 mb-2 w-40 md:w-32 h-60 rounded-xl p-1'
             />
         </span>
        </Link>
         <div className=''>
        ...
        <p className='text-sm text-gray-400'>{post.views} * {post.date}</p>
            ...
            <div onClick={handleIcon} className={` flex flex-col  sm:w-64 rounded-xl p-4 relative -mb-96 -top-72 bg-filter -translate-y-10 left-32 z-10 ${icon ? "block" : "hidden"}`}>
