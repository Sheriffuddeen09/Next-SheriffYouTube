...
export default function PostYou(post:Posts){
    ...
    return(
      <div>
        ...
        <div className='flex -mx-3 flex-col lg:flex-col md:flex-row md:gap-4 my-1.5 text-sm'>
        <Link href={`/edityou/${post.id}`}>
             <div className='width hidden sm:block translate-x-3'>
             <Video src={post.video} width={330} className=' mb-2 w-40 md:w-32 h-40 rounded-xl p-1'
             />
               <div className='w-60 bg-relative relative bottom-40  transition-all duration-500 ease-in-out  -translate-y-2 -mb-40 h-40 rounded-xl'>
            ...
            <div className='w-80 block sm:hidden -translate-x-2'>
        ...
        <div className='flex flex-row justify-between gap-0 '>
         ...
         <p className='text-sm'>
            { 
                ...
                post.title : `${(post.title).slice(0,24)}...`
        ...
        <p className='text-sm text-gray-400'>{post.views} * {post.date}</p>
            ...
            <div onClick={handleIcon} className={`flex flex-col  sm:w-64 rounded-xl px-4 pt-20 pb-16 relative -mb-96 -top-80 bg-filter -translate-y-10 left-32 z-10 ${icon ? "block" : "hidden"}`}>
