...
export default function TodoId (post:Posts){
...
/*
    const handleDelete = async (e: MouseEvent<HTMLButtonElement>) => {
        ...
        const res = await fetch(`http://127.0.0.1:3500/todos/${post.id}`, {
            method: 'DELETE',
            headers: {
...
return(
    <div>
      ...
      <div className='flex flex-col  px-8 lg:flex-col md:flex-row md:gap-4 my-1.5 text-sm'>
           <span className=''>
           <Video src={post.video} width={600} className='bg-gray-50 w-96 mb-2 h-96 rounded-xl'
           />
       </span>
       <div>
      ...
      <p className='text-sm text-gray-400'>{post.views} * {post.date}</p>
          ...
          <div onClick={handleIcon} className={` flex flex-col  sm:w-64 rounded-xl p-4 relative bottom-96 bg-filter -translate-y-10qqqqqqqqqqqqq left-40 z-10 ${icon ? "block" : "hidden"}`}>
